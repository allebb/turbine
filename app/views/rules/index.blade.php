...
<table class="table table-hover">
    ...
    <tr class="rule_{{ $rule->id }}"><td>{{ TurbineUtil::LinkOriginAddresses($rule->hostheader) }}</td><td>@if($rule->enabled)<span class="glyphicon glyphicon-ok"></span>@endif</td><td>{{ $rule->targets }}</td><td>@if($rule->nlb)<span class="glyphicon glyphicon-ok"></span>@endif</td><td><a href="{{{ URL::route('rules.edit', $rule->id) }}}" class="btn btn-xs btn-info">Edit</a> <button onclick="delete_rule({{ $rule->id }})"  class="btn btn-xs btn-danger">Delete</button></td></tr>
    @endforeach
</table>
@else
<p>&nbsp;</p>
