    ...
    <body>
        ...
        <div class="container theme-showcase">
            ...
            <table class="table table-hover">
                ...
                <tr><td>{{ TurbineUtil::LinkOriginAddresses($rule->hostheader) }}</td><td>@if($rule->enabled)<span class="glyphicon glyphicon-ok"></span>@endif</td><td>{{ $rule->targets }}</td><td>@if($rule->nlb)<span class="glyphicon glyphicon-ok"></span>@endif</td><td><a href="{{{ URL::route('rules.edit', $rule->id) }}}" class="btn btn-xs btn-info">Edit</a> <a href="{{ URL::route('rules.edit', $rule->id) }}#delete"  class="btn btn-xs btn-danger">Delete</a></td></tr>
                @endforeach
            </table>
            @else
            <p>&nbsp;</p>
